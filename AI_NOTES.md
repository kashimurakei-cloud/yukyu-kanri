...
- 残高オーバー: calcBalanceのoverflowMin(どの付与からも引けなかった分)。**残とは相殺しない**(翌年付与から引かない)。別枠で表示し、給与側で対応する運用。スタッフ入力時は超過警告+confirm、ヒーローに⚠表示、院長側は履歴カード・職員カードのタグ・ホーム要対応に表示。代理フォームは注意のみ(過去分は現在残と比較できないため)。
