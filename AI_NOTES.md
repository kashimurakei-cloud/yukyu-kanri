...
- 残高オーバー: calcBalanceのoverflowMin(累計)とrecentOverflowMin(直近付与日以降)。**残とは相殺しない**(翌年付与から引かない)。別枠で表示し、給与側で対応する運用。スタッフのヒーロー・職員カードのタグ・ホーム要対応はrecent(新しい付与が来たら消える)、院長の履歴カードは累計を過去情報として残す。スタッフ入力時は超過警告+confirm。代理フォームは注意のみ。
