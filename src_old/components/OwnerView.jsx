  ...
  return (
    <>
      ...
      {!loading && tab === "overview" && (
        <OverviewTab staffList={staffList} recordsByStaff={recordsByStaff} pendingPlanned={pendingPlanned} />
      )}
...
/* ------- 全員の状況 ------- */
function OverviewTab({ staffList, recordsByStaff, pendingPlanned }) {
...
function StaffHistoryCard({ staff, records, onClose }) {
  ...
  return (
    <section style={{ ...S.card, border: "1.5px solid #3a7d6e" }}>
      ...
      <p style={S.noteSmall}>院長は閲覧のみ。修正が必要なときは取得登録から行います。</p>
    </section>
  );
}
