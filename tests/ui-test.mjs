...
try {
  ...
  setVal([...rootEl.querySelectorAll('input[type="date"]')].pop(), "2025-06-02");
  await wait(150);
  html = rootEl.innerHTML;
  console.log("2025年以前→日単位の注意:", html.includes("日数で入力"));
  console.log("2025年以前→分入力は無効:", html.includes("分で入力（2026年〜）"));
  ...
  setVal([...rootEl.querySelectorAll('input[type="date"]')].pop(), "2026-02-02");
  await wait(150);
  console.log("2026年以降→注意が消える:", !rootEl.innerHTML.includes("日数で入力してください"));
